				  ...
				  while($row = mysqli_fetch_assoc($result)){
						 ...
						 if($stat == 0){
							 if($fg == $ans){
							 	//update flag status
							 	$suc = 1;
							 	$update_status_sql = mysqli_query($connection,"UPDATE ".$c->getGameId()."_secgenflag SET STATUS='$suc' WHERE FLAG='$fg' AND TEAM='$team'");
								if($update_status_sql){
									...
									$calPoints = mysqli_query($connection, "SELECT HINT_TYPE FROM ".$c->getGameId()."_hint WHERE C_ID='$cid' AND SYSTEM_NAME='$vm' AND TEAM='$team' AND HINT_STATUS='1'");
									$points = 0;
											...
											if($log_sql){								
												...
												if(mysqli_num_rows($revealHint) > 0){
													while($hID = mysqli_fetch_assoc($revealHint)){
														$id = $hID['HINT_ID'];
														mysqli_query($connection, "UPDATE ".$c->getGameId()."_hint SET HINT_STATUS='1' WHERE HINT_ID='$id' AND TEAM='$team'");
								...
								mysqli_query($connection, "UPDATE updater SET ACTIVITY='1' WHERE TEAM='$team' AND GAME_ID='$c->getGameId()'");
							 } 
						}else{
